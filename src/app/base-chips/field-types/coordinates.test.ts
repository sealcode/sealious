...
describe("coordinates", () => {
	...
	it("should encode provided values in the form of: string, object and tuple correctly to geoJSONObject", async () => {
		await withRunningApp(extend, async ({ app }): Promise<void> => {
			const geoJSONObjectExample = { type: "Point", coordinates: [12.12421, 65.21312] };
			const res = await coordsInstance.encode(new app.SuperContext(), testCoordsString);
			assert.deepStrictEqual(res, geoJSONObjectExample);
			const res2 = await coordsInstance.encode(new app.SuperContext(), testCoordsObject);
			assert.deepStrictEqual(res2, geoJSONObjectExample);
			const res3 = await coordsInstance.encode(new app.SuperContext(), testCoordsTuple);
