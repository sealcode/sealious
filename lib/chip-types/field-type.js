...
function FieldType (declaration) {
	Chip.call(this, "field_type", declaration.name);
...
FieldType.prototype = new function(){
	...
	this.process_declaration = function(declaration){
		...
		this.is_subject = Boolean(declaration.is_subject);
		if (this.is_subject){
			this.get_child_subject = declaration.get_child_subject;
	...
	this._format = function(context, field_params, decoded_value, format_params){
	...
	this.full_text_search_enabled = function(params){
		if (this.declaration.full_text_search_enabled){
			return this.declaration.full_text_search_enabled(params);
		} else if (this.extends){
			return this.parent_field_type.full_text_search_enabled(params);
		} else {
			return false;
		}
	}

	this.format = function(context, params, decoded_value, format_params){
		if (this.declaration.format){
			return Promise.resolve(this.declaration.format(context, params, decoded_value, format_params));
		} else if (this.declaration.extends){
			return this.parent_field_type.format(context, params, decoded_value, format_params);
		} else {
			return this.get_method("format")(context, params, decoded_value, format_params);
		}
	}
}
