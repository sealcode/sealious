		...
		return new Promise(function(resolve, reject){
			...
			var new_reject = function(error){
				if (typeof error === "string"){
					reject(new Sealious.Errors.ValidationError(error));
				} else {
					reject(error);
				}
			}
...
var FieldType = function(declaration){
	...
	if (declaration instanceof FieldType){
		...
		return Sealious.ChipManager.get_chip("field_type", declaration);
	} else if (declaration.extends){
		var parent_field_type = Sealious.ChipManager.get_chip("field_type", declaration.extends);
	...
	if (declaration.name){
		Sealious.ChipManager.add_chip("field_type", declaration.name, self);
	}
