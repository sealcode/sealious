...
// 	collection: 	require("./resource-type.js"),
...
const chip_type_start_order = ["access_strategy", "field_type", "collection", "channel"];
...
const ChipManager = {
	...
	*/
	...
	add_chip: function(type, name, chip){
		if (chips[type] === undefined){
			chips[type] = [];
		}
		if(chips[type][name]) throw new Error(`Chip '${type}.${name}' already exists!`);
	...
	*/
	__get_all_collections: function(chips){
		const names = [];
	...
	get_all_collections: function(){
		return this.__get_all_collections(chips);
	},
	/**
	...
	*/
	chip_exists: function(type, name){
	...
	__get_chip: function(chips, type, name){
		try {
			...
			throw new Error(`ChipManager was asked to return a chip of type "${type}" and name "${name}", but it was not found`);
		}
	},
	get_chip: function(type, name){
	...
	*/
	__get_chip_by_longid: function(chips, longid){
		const type = longid.split(".")[0];
		const name = longid.split(".")[1];
		return this.__get_chip(chips, type, name);
	},
	get_chip_by_longid: function(longid){
		return this.__get_chip_by_longid(chips, longid);
	},
	__get_chip_amount_by_type: function(chips, type){
		if (chips[type]){
	...
	get_chip_amount_by_type: function(type){
	...
	*/
	...
	__get_chips_by_type: function(chips, chip_type){
		return chips[chip_type];
	},
	get_chips_by_type: function(chip_type){
		return this.__get_chips_by_type(chips, chip_type);
	}
};
