...
ResourceType.prototype = new function(){
	...
	this.validate_field_values = function(context, values, old_values){
		var that = this;
		var validation_errors = {};
		...
		for(var key in values){
			if(validation_errors[key]===undefined){
				...
				var old_value = old_values && old_values[key];
				var validation_promise = field.check_value(context, value, old_value);
	...
	this.encode_field_values = function(context, body){
		...
		for(var field_name in body){
			...
			promises[field_name] = this.keys[field_name].encode_value(context, current_value);
		}
