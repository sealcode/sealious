	...
	this.validate_field_values = function(context, check_missing, values, old_values){
		...
		return Promise.all(validation_tasks)
			.then(function(){
				...
				if (has_errors) {
					var error_details = "";
					for (var prop in validation_errors) {
						error_details = error_details + " * " + prop + " - " + validation_errors[prop] + "\n";
					}
					return Promise.reject(new Sealious.Errors.ValidationError("There are problems with some of the provided values: \n" + error_details, validation_errors));
				} else {
					return Promise.resolve();
				}
			});
