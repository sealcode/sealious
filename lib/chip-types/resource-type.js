...
ResourceType.prototype = new function(){
	...
	this.set_access_strategy = function(strategy_declaration){
		...
		if (typeof strategy_declaration === "string"){
			...
			if (strategy_declaration instanceof Sealious.AccessStrategy){
				this.access_strategy = {
					"default": strategy_declaration
				}
			} else {
				for (var action_name in strategy_declaration){
					var access_strategy;
					if (typeof strategy_declaration[action_name] === "string"){
						access_strategy = Sealious.ChipManager.get_chip("access_strategy", strategy_declaration[action_name]);
					} else if (strategy_declaration[action_name] instanceof Sealious.AccessStrategy){
						access_strategy = strategy_declaration[action_name];
					}
	...
	this.get_resource_representation = this.decode_db_entry = function(context, db_document){
		var representation = {};
		...
		representation.body = this.decode_values(context, db_document.body);
