...
Collection.pure = {
	...
	get_missing_required_field_values: function(fields, new_values){
		...
		for(const field_name in new_values){
			if(fields[field_name] && fields[field_name].required && new_values[field_name]===""){
				errors[field_name] = new Errors.ValidationError(`Missing value for required field: '${  field_name  }'`);
			}
		}
		return errors;
	},
	validate_field_values: function(field_type_name, fields, context, assume_delete_value_on_missing_key, new_values, old_values){
		const errors_array = [
			Collection.pure.get_missing_required_field_values(fields, new_values),
	...
	_get_calculated_fields: function(context, calculated_fields, representation, raw_db_entry, calculate){
		const ret = {};
	...
	get_resource_representation: function(fields, field_type_name, context, db_document, format, calculated_fields, calculate){
		...
		if(calculate){
			representation.calculated_fields = Collection.pure._get_calculated_fields(context, calculated_fields, representation, db_document, calculate);
		}
	...
	get_aggregation_stages: function(access_strategy_map, fields, context, action_name, query_params){
		...
		return Promise.all([
			...
			Object.keys(fields).map(
				(field_name)=>fields[field_name].get_aggregation_stages(context, query_params)
			),
		])
		.map(Promise.all)
