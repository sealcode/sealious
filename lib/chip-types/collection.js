    ...
    if (declaration) {
        ...
        if (declaration.calculated_fields) {
            for (const calc_field_name in declaration.calculated_fields) {
                const calc_field_type_declaration = declaration.calculated_fields[
                    calc_field_name
                ][0];
                const calc_field_type_params = declaration.calculated_fields[
                    calc_field_name
                ][1];
...
Collection.pure = {
    add_field: function(app, field_type, fields, field_declaration) {
        ...
        if (!fields[field_name]) {
            ...
            throw new Errors.DeveloperError(
                `Duplicate field names: "${field_name}" in collection: "${field_type.name}"`
            );
        }
    },
    ...
    get_missing_values_checker: function(
        ...
        if (assume_delete_value_on_missing_key) {
            return function(field_name) {
                return fields[field_name].required &&
    ...
    validate_field_values: function(
        ...
        return Promise.all(errors_array).reduce(merge).then(function(errors) {
            const user_errors = {};
    ...
    get_access_strategy: function(access_strategy_map, action_name) {
        const ret = access_strategy_map[action_name] ||
            access_strategy_map["default"];
        return ret;
    },
    ...
    _get_body: function(fields, context, db_document, format) {
        const decoded_values = Collection.pure.decode_values(
            fields,
            context,
            db_document.body
        );
...
Collection.prototype = {
    ...
    get_aggregation_stages(context, action_name, query_params) {
