...
const Collection = function(declaration){
	if (typeof declaration === "string"){
		return ChipManager.get_chip("collection", declaration);
			...
			for (let i = 0; i < declaration.fields.length; i++){
				if (declaration.fields[i].params){
					if (typeof declaration.fields[i].params !== "object"){
						throw new Boom.badRequest(`In field "${declaration.fields[i].name}": "params" is of wrong type, it should be an object`);
					}
				}
			}
...
Collection.prototype.add_field = function(field_declaration){
	...
	if (!this.fields[field_name]){
		...
		throw new Boom.badRequest(`Duplicate field names: "${field_name}" in collection: "${this.name}"`);
	}
};
...
Collection.prototype.get_unknown_field_errors = function(values){
	...
	for (const field_name in values){
		if (this.fields[field_name] === undefined){
			validation_errors[field_name] = new Boom.badRequest(`Unknown field '${field_name}' for collection '${this.name}`);
		}
	}
...
Collection.prototype.get_missing_field_values_errors = function(values, assume_delete_value_on_missing_key, old_values){
	...
	.each(function(field_name){
		...
		errors[field_name] = new Boom.badRequest(`Missing value for field "${field_name}"`);
	}).then(function(){
		return errors;
	});
};
...
Collection.prototype.validate_field_values = function(context, assume_delete_value_on_missing_key, new_values, old_values){
	...
	.then(function(errors){
		...
		if (user_errors_amount > 0){
			throw new Boom.badRequest("There are problems with some of the provided values.", user_errors);
		}
	});
};
...
Collection.prototype.get_resource_representation = Collection.prototype.decode_db_entry = function(context, db_document, format){
	...
	representation.collection_name = this.name;
