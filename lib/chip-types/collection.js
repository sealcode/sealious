...
Collection.pure = {
	...
	_get_calculated_fields: function(context, calculated_fields, representation){
		const ret = {};
		for(const field_name in calculated_fields){
			ret[field_name] = calculated_fields[field_name].get_value(context, representation);
		}
		return Promise.props(ret);
	},
	get_resource_representation: function(fields, field_type_name, context, db_document, format, calculated_fields){
		...
		representation.calculated_fields = Collection.pure._get_calculated_fields(context, calculated_fields, representation);
	...
	get_aggregation_stages: function(fields, context, query_params){
		return Promise.all(
...
Collection.prototype = {
	...
	get_resource_representation(context, db_document, format){ return pure.get_resource_representation(this.fields, this.name, context, db_document, format, this.calculated_fields);},
	...
	get_aggregation_stages(context, query_params){return pure.get_aggregation_stages(this.fields, context, query_params);}
};
