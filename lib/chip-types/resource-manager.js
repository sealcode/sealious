 ...
 */
 var ResourceManager = new function(){
	 	...
	 	return new Promise(function(resolve,reject){
	 		...
	 		.then(function(documents){
	 			if (documents[0] === undefined) {
	 				reject(new Errors.NotFound("resource of id " + resource_id + " not found"));
	 			} else {
	 				var database_entry = documents[0];
	 				var resource = new ResourceRepresentation(database_entry);
	 				resolve(resource.getData());
	...
	this.get_resource_type_signature = function(type_name){
		return new Promise(function(resolve, reject){
			...
			if(resource_type_chip){
				...
				reject(new Errors.Error("ResourceManager tried to access chip type `" + type_name + "`, which does not exist."));
			}
		})
	}
}
