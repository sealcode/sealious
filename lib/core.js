	...
	this.check_version = function(package_info){
		...
		var check_request = http.get(url, function(res){
			...
			res.on('end', function(){
				...
				console.log(sealious_npm);
				if(sealious_npm.error){
					Sealious.longogger.warning("npm registry error when requesting info for version " + version + ": " + sealious.npm.error);
				}else if(!sealious_npm.version){
					Sealious.longogger.warning("unknown npm registry error when requesting info for version " + version + ".");
				}else{
