...
const SealiousCore = {
	...
	load_custom_config: function(){
		...
		console.log("Looking for custom custom config from file in:", default_file_path);
		...
		try {
			...
			if(e.name === "YAMLException"){
				console.error("Syntax error in sealious.yml:\n" + e.message);
				process.exit(1);
			} else if (e.code === "ENOENT"){
				console.log("\n\n\tNo 'sealious.yml' file detected. Consider creating it alongside your app's package.json file if you want to configure the app's chips.\n\n");
			} else {
				console.error(e);
				process.exit(1);
			}
