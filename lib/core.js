...
const SealiousCore = {
	get_default_config: function(){
		...
		const configFile = path.join(sealious_dir, "default_config.yml");
		const defaultConfig = yaml.safeLoad(fs.readFileSync(configFile, "utf8")).core;

		for (const i in defaultConfig){
			ConfigManager.set_default_config(i, defaultConfig[i]);
		}
	},
