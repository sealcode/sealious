const get_matching_roles_counter = async function(app, context, allowed_roles) {
	const user_id = context.user_id;

	const user_roles = await app
		.run_action(
		...
		.map(role_resource => role_resource.body.role);

	return allowed_roles.filter(allowed_role => user_roles.includes(allowed_role))
		.length;
};
