...
ConfigManager.pure = {
	...
	load_custom_config: function(default_config, config, app_dir) {
		...
		const config_result = cosmiconfig("sealious", {
			rc: "sealious.yml",
			sync: true,
			rcExtensions: true
		}).load(
			path.resolve(locreq_curry(app_dir).resolve("sealious.yml"), "../")
		);
		log("Looking for custom config from file in:", config_result.filepath);
		log("-----");
		log("custom config:", "\n", config_result.config);
		log("-----");
		return ConfigManager.pure.set_config(
			default_config,
			config,
			config_result.config
		);
	...
	set_config: function(default_config, config, key, new_config) {
		if (arguments.length === 3) {
			...
			return ConfigManager.pure.modify_config(
				default_config,
				config,
				false,
				key,
	...
	get_configuration: function(default_config, config, key) {
		...
		while (key_elements.length) {
			...
			to_merge_left =
				to_merge_left[current_key] === undefined
					? {}
					: to_merge_left[current_key];
			to_merge_right =
				to_merge_right[current_key] === undefined
					? {}
					: to_merge_right[current_key];
		}
...
ConfigManager.prototype = {
	apply_changes: function(config_array) {
		...
		if (config_array instanceof Array) {
			...
			throw new Error(
				`Bad configuration array:${config_array.toString()}`
			);
		}
	},
	load_default_config_for_dir: function(dir) {
		return this.apply_changes(
			ConfigManager.pure.load_default_config_for_dir(
				this.default_config,
				this.config,
				dir
			)
	...
	load_custom_config: function(app_dir) {
		return this.apply_changes(
			ConfigManager.pure.load_custom_config(
				this.default_config,
				this.config,
				app_dir
			)
	...
	set_default_config: function(key, value) {
		return this.apply_changes(
			ConfigManager.pure.set_default_config(
				this.default_config,
				this.config,
				key,
				value
	...
	get_configuration: function(key) {
		return ConfigManager.pure.get_configuration(
			this.default_config,
			this.config,
			key
		);
	...
	modify_config: function(is_default_config, key, value) {
		return this.apply_changes(
			ConfigManager.pure.modify_config(
				this.default_config,
				this.config,
				is_default_config,
				key,
