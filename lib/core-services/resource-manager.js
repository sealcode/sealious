	 ...
	 */
	 ...
	 this.list_by_type = function(context, type_name){
		...
		if(access_strategy.item_sensitive){
			preliminary_access_strategy_check = new Promise(function(resolve, reject){
				resolve();
			})
		}else{
		...
		return preliminary_access_strategy_check.then(function(){
			...
			console.log(access_strategy);
			if(access_strategy.item_sensitive){
				var filter_individual_items = Promise.filter(ret, function(item){
					return new Promise(function(resolve, reject){
				...
				return filter_individual_items;
			}else{
