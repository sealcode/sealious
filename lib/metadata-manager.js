...
var MetadataManager = new function(){
	...
	this.get_value = function(key, dispatcher){
		return new Promise(function(resolve, reject){
			dispatcher.database_query("meta", "find", { "key": key }, {}).then(function(response){
	...
	this.set_value = function(key, value, dispatcher){
		return new Promise(function(resolve, reject){
			that.has_key(key, dispatcher).then(function(has){
				function actual_set(){
					dispatcher.database_query("meta", "update", { key: key }, {key:key, value: value}).then(function(response){
						if(response.length==0){
							resolve(false);
						}else{
							resolve(true);
				...
				if(!has){
					dispatcher.database_query("meta", "insert", {key: key, value: value}, {}).then(function(data){
						actual_set();
					})
				}else{
					actual_set();
