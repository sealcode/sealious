...
function preprocess_resource_filter (context, resource_type, filter) {
	var processed_filter = {};
	for (var field_name in filter){
		if (resource_type.fields[field_name]){
			var encoded_value = resource_type.fields[field_name].encode_value(context, filter[field_name]);
...
ResourceTypeCollection.prototype.list_resources = function(context, filter){
	var self = this;
	...
	.then(function(){
		return preprocess_resource_filter(context, self.resource_type, filter)
