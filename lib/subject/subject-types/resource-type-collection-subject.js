...
ResourceTypeCollection.prototype._preprocess_resource_filter  = function(context, filter){
	...
	for (var field_name in expanded_filter){
		if (self.resource_type.fields[field_name]){
			var field = self.resource_type.fields[field_name];
...
ResourceTypeCollection.prototype.list_resources = function(context, params){
	...
	if (params.sort){
		...
		for (var i in full_sort_params){
			switch (full_sort_params[i]){
				case "desc":
					full_sort_params[i] = -1;
					break;
				case "asc":
					full_sort_params[i] = 1;
					break;
				default:
					delete full_sort_params[i];
			}
		}
	...
	.map(function(db_document){
		return self.resource_type.get_resource_representation(context, db_document, params.format)
