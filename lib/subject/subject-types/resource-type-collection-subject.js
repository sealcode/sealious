...
function preprocess_resource_filter (filter) {
	var processed_filter = {};
	for (var i in filter){
		processed_filter[i] = {$eq: filter[i]}
	}
...
ResourceTypeCollection.prototype.list_resources = function(context, filter){
	...
	.then(function(){
		return Sealious.Datastore.find("resources", {type: self.resource_type.name, body: preprocess_resource_filter(filter)}, {}, {});
	})
