...
var ResourceTypeFieldSubject = require("./resource-type-field-subject.js");
...
SingleResource.prototype.get_resource = function(context, args){
	...
	.then(function(db_entries){
		if (db_entries[0] === undefined) {
			...
			return self.resource_type.get_resource_representation(context, db_entries[0], args.format);
		}
	}).then(function(resource_representation){
		return self.resource_type.check_if_action_is_allowed(context, "retrieve", resource_representation)
		.then(function(){
...
SingleResource.prototype.edit_resource = function(context, values_to_patch, delete_empty_values){
	...
	.then(function(resource_data){
		...
		return self.resource_type.check_if_action_is_allowed(context, "update", resource_representation)
	}).then(function(){
...
SingleResource.prototype.delete_resource = function(context, args){
	...
	return self.get_resource(context)
	.then(function(resource_representation){
		return self.resource_type.check_if_action_is_allowed(context, "delete", resource_representation);
	}).then(function(){
