...
SingleResource.prototype.edit_resource = function(context, values_to_patch, delete_empty_values){
	...
	var values_getter = CachedGetter(function(){
		return item_getter().then(function(item){
			return item.body;
		})
	})
	...
	.then(function(){
		return self.resource_type.validate_field_values(context, delete_empty_values, values_to_patch, values_getter)
	}).then(function(){
		return self.resource_type.encode_field_values(context, values_to_patch, values_getter);
...
SingleResource.prototype.delete_resource = function(context, args){
	...
	return this.resource_type.check_if_action_is_allowed(context, "delete", item_getter)
	.then(function(){
...
SingleResource.prototype.perform_action = function(context, action_name, args){
	console.log("action_name=='edit'", action_name=='edit');
	switch (action_name){
		case "show":
