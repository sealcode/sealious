...
const SingleResource = function(collection, resource_id){
...
SingleResource.prototype.__edit_resource = function(datastore, collection, resource_id, context, values_to_patch, delete_empty_values){
	...
	.then(function(resource_data){
		...
		return datastore.update(collection.name, {sealious_id: self.resource_id}, {$set: query});
	}).then(function(){
		return SingleResource.prototype.__get_resource(datastore, collection, resource_id, context, self.resource_id);
	});
};
