...
function format_filename(original_filename, format_name) {
    return original_filename.split(".").slice(0, -1).join(".") +
        "-" +
        format_name +
        ".jpg";
}
...
const ImageFormat = function(app, file_id, format_name) {
    ...
    function create_formatted_version(file_id, format_name) {
        const format_obj = app.ConfigManager.get_config().image_formats[
            format_name
        ];
        return app.Datastore
            .find("files", { id: file_id })
            .then(function(matches) {
                ...
                return Promise.promisify(resize_request.write, {
                    ...
                    .then(function(file) {
                        return Promise.promisify(fs.unlink)(
                            temp_file_path
                        ).then(() => file);
                    });
            });
