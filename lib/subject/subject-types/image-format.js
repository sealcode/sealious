...
const ImageFormat = function(app, file_id, format_name) {
	...
	function create_formatted_version(file_id, format_name) {
		...
		return app.Datastore.find("files", { id: file_id }).then(function(matches) {
			...
			return Promise.promisify(resize_request.write, {
				...
				.then(function(file) {
					return Promise.promisify(fs.unlink)(temp_file_path).then(() => file);
				});
		});
	}

	function get_formatted_version(file_id, file_name, format_name, format_obj) {
		const random = Math.random();
		const hash = format_hash(format_obj);
		return app.Datastore.aggregate("formatted-images", [
			{
				$match: {
					"body.original_photo_file": { $eq: file_id },
				},
			},
			{
				$match: {
					$or: [{ "body.format.original": hash }, { "body.format.safe": hash }],
				},
			},
			...
			return (
				results[0] &&
				results[0].body && {
					id: results[0].body.formatted_photo_file,
					...
					return get_formatted_version(
						...
						.then(function(result) {
							if (result !== undefined) {
								...
								return create_formatted_version(file_id, format_name);
							}
						})
						.then(file_description => {
							let ret = new app.Sealious.File.from_db_entry(file_description);
