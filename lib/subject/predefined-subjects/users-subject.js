...
const UsersSubject = function(app){
	this.perform_action = function(context, action_name, params){
		...
		switch (action_name){
		case "create":
			return app.run_action(context, ["collections", "user"], "create", params);
		case "show":
			return app.run_action(context, ["collections", "user"], "show", params);
		default:
	...
	this.get_child_subject = function(key){
		if (me_synonyms.indexOf(key) !== -1){
			...
			const username = key;
			return app.run_action(new SuperContext(), ["collections", "user"], "show", {filter:{username:username}})
			.then(function(result){
				if (result.length === 0){
					throw new Errors.BadSubjectPath(`Unknown username: '${username}'`);
				} else {
					const user = result[0];
					return RootSubject.get_subject(["collections", "user", user.id]);
				}
			});
		}
	};
