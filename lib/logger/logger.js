...
function getDateTime(with_date) {
    ...
    if (!with_date) { return hour + ":" + min + ":" + sec + "." + milli; }
    else { return year + "-" + month + "-" + day + " " + hour + ":" + min + ":" + sec + "." + milli; }
...
var winston_level = new (winston.transports.Console)({ level: logger_level, colorize: logger_color, timestamp: function() { return getDateTime(false); }  });
transports_array.push(winston_level);

if (logger_file) {
    var winston_save_to_file = new (winston.transports.File)({ filename: logger_file, colorize: logger_color, timestamp: function() { return getDateTime(true); }, dirname: logger_dirname, json: logger_to_json });
