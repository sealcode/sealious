	...
	is_proper_value: function(context, params, new_value){
		if (params === undefined || (params.max_length === undefined && params.min_length === undefined)){
			...
			if (new_value.length >= params.min_length){
				...
				return Promise.reject(`Text '${new_value}' is too short, minimum length is ${params.max_length} chars.`);
			}
		}
			...
			if (new_value.length >= params.min_length && new_value.length <= params.max_length){
				...
				return Promise.reject(`Text '${new_value}' length should be between ${params.min_length} and ${params.max_length}.`);
			}
		}
	},
