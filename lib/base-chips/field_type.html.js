...
var utils = {
	is_allowed: function(set_keep, set_remove, default_decision, element) {
		...
		if (!set_keep.has(element) && !set_remove.has(element) || set_keep.has(element) && set_remove.has(element)) return default_decision;
		else if (set_keep.has(element) && !set_remove.has(element)) return true;
		else if (!set_keep.has(element) && set_remove.has(element)) return false;
	},
	...
	merge_params: function(params) {
		if (params !== undefined && params.tags !== undefined && params.attributes !== undefined) {
			var tags_dec = params.tags.default_decision;
			var attr_dec = params.tags.default_decision;
			if (tags_dec !== "keep" && tags_dec !== "remove" || tags_dec === undefined) params.tags.default_decision = "remove";
			if (attr_dec !== "keep" && attr_dec !== "remove" || attr_dec === undefined) params.attributes.default_decision = "remove";
		}
