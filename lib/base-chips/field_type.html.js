...
var utils = {
	...
	parse: function(input, sets, default_decision_tags, default_decision_attributes, default_decision_tag_content) {
		...
		var parser = new htmlparser.Parser({
			...
			ontext: function(text) {
				var last_tag = tags_stack[tags_stack.length - 1];
				if (sets.tag_content.keep.has(last_tag)) output += text;
			},
	...
	merge_params: function(params) {
		...
		if (params !== undefined && params.tag_content !== undefined) {
			var a = params.tag_content;
			if (a.default_decision !== "keep" && a.default_decision !== "remove" || a.default_decision === undefined) {
				params.tag_content.default_decision = "remove";
			}
		}
