...
var utils = {
	...
	create_sets_from_params: function(params) {
		var params_sets = {}
		params_sets.tags_keep = new Set(params.tags.keep);
		params_sets.tags_remove = new Set(params.tags.remove);
		params_sets.attributes_keep = new Set(params.attributes.keep);
		params_sets.attributes_remove = new Set(params.attributes.remove);
		return params_sets;
	}
}
	...
	decode: function(context, params, value_in_database) {
		...
		var parser = new htmlparser.Parser({
			onopentag: function(name, attribs) {
				var decision = utils.check_fragment(params.tags_keep, params.tags_remove, default_decision_tags, name);
				if (decision !== "remove") {
					output += "<" + name + " ";
					for (key in attribs) {
						if (utils.check_fragment(params.attributes_keep, params.attributes_remove, default_decision_attributes, key) === "keep") {
			...
			onclosetag: function(tagname) {
				var decision = utils.check_fragment(params.tags_keep, params.tags_remove, default_decision_tags, tagname);
