...
var db_rest_api_client = new function(){
	...
	this.query = function(collection_name, mode, query, options){
		...
		return new Promise(function(resolve, reject){
			...
			if(mode=="insert"){
				var url = generateUrl() + "/api/rest/v1/" + collection_name + "/";
				console.log("query w insert", query);
				var query_string = {
				  mode: mode,
				  query: JSON.stringify(query),
				  options: JSON.stringify(options)
				...
				var query_string_json = JSON.stringify(query_string);
				request.post(url, {form:{json_encoded_payload:query_string_json}}, function(err, res, body){
					console.log("REQUEST RESPONSE BODY:", body);
					console.log("error:", err)
					if(err){
						throw new Error(err);
