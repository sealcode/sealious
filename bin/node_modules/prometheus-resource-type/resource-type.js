		...
		return new Promise(function(resolve, reject){
			...
			for(var given_field_name in field_values){
				if(!(that.fields[given_field_name])){
					var error_message = "===error=== type `" + that.name + "` does not have a field with a name `" + given_field_name + "`";
					var err = new Error(error_message);
					err.err_message = error_message;
					err.err_code = "unknown_field_name";
					reject(err);
					return;
				}
			}
			...
			for(field_name in that.fields){
				...
				if(current_field.required && field_values[field_name]===undefined){
					var error_message = "===error=== value for field `" + field_name + "` in `" + that.name + "` is required, but not set";
					var err2 = new Error(error_message);
					err2.err_message = error_message;
					err2.err_code = "missing_field_value";
					reject(err2);
					return;
				}
			}
