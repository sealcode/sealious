...
var ModuleManager = new function(){
	...
	this.get_module_count = function(){
		var c = 0;
	...
	this.register_modules = function(modules_dir){
		...
		for(var i in modules_dir_contents){
			...
			var current_module = new Module(current_module_path);
	...
	function decide_module_execution_order(){
		...
		var accepted_modules_count = module_execution_order.length;
		var modules_total = modules_by_id.length;
	...
	function deduce_dependency_issues(){
		...
		for(var i in modules_by_id){
			for(var j in modules_by_id){
				if(i!=j && intersection_of_arrays(modules_by_id[i].requires, modules_by_id[j].defines).length && intersection_of_arrays(modules_by_id[j].requires, modules_by_id[i].defines).length){
	...
	this.prepare_chips_for_all_modules = function(chip_types){
		var module_execution_order = decide_module_execution_order();
		for(var id in module_execution_order){
			var current_module = modules_by_id[id];
			current_module.prepare(chip_types_to_start);
		}
	}
