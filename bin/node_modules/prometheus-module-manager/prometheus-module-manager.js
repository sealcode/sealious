...
Module.prototype = new function(){
	...
	this.getDefindedChipsIDsByType = function(type) {
		...
		var chips = module_info[this.id]["prometheus-defined-chips"].toString().split(",");
			...
			if(!current_module_info.registred){
				...
				for (var i=0; i<current_module_info.defines.length; i++){
					...
					if (registered_chips.has(current_chip_id)){
						throw new Error("Chip " + current_chip_id + " has already been registered by module " + was_registred_by[current_chip_id] + ", so module " + current_module_id + " can not be executed.");
					}
				}
				if(can_execute){
