	...
	www_server.route({
		...
		handler: function(request, reply) {
			dispatcher.users_password_match(request.payload.username, request.payload.password).then(function(response){
				console.log("http_session", response);
				if (response[0]){
					var sessionId = sessionManager.new_session(response[0].userdata_id);
					reply("http_session: Logged in!").state('PrometheusSession', sessionId).redirect('http://localhost/user.html#'+response[0].userdata_id);
				}else{
