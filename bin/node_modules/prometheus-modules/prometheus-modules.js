...
function decide_module_execution_order(){
	...
	while(!all_modules_registred){
		...
		for(var module_id in module_info){
			if(registred_ids.has(module_id)){
				continue;
			}
			current_module_info = module_info[module_id];
			if(!current_module_info.registred){
				...
				for (var i=0; i<current_module_info.defines.length; i++){
					...
					if (registered_injectables.has(current_injectable_id)){
						throw new Error("Injectable " + current_injectable_id + " has already been registered.");
					}
				}
				if(can_execute){
					module_ids_in_order.push(module_id);
					registred_ids.add(module_id);
