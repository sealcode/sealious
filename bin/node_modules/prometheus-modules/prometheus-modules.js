...
function decide_module_execution_order(scope){
	...
	var all_modules_executed = false;
	var module_ids_in_order = [];
	var defined_injectables = new Set();
	var executed_ids = new Set();
	while(!all_modules_executed){
		all_modules_executed = true;
		for(var i in module_info){
			if(executed_ids.has(i)){
				continue;
			}
			...
			if(!current_module_info.registred){
				...
				if(!current_module_info.requires){
					...
					for(var j in current_module_info.requires){
						if(!defined_injectables.has(current_module_info.requires[j])){
							can_execute = false;
						}
					}
				}
				if(can_execute){
					...
					executed_ids.add(i);
					defined_injectables.addAll(module_info[i].defines);
				}else{
					all_modules_executed = false;
				}
			}
		}
	}
