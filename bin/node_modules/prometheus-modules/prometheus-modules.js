...
function Module(module_path){
	...
	settings.defines = settings["prometheus-defines"] || [];
...
function decide_module_execution_order(){
	...
	var registered_injectables = new Set();
			...
			if(!current_module_info.registred){
				...
				if(!current_module_info.requires){
					...
					for(var j in current_module_info.requires){
						if(!registered_injectables.has(current_module_info.requires[j])){
							can_execute = false;
						}
					}
				}
				for (var i=0; i<current_module_info.defines.length; i++){
					var current_injectable_id = current_module_info.defines[i];
					if (registered_injectables.has(current_injectable_id)){
						throw new Error("Injectable " + current_injectable_id + " has already been registered by module " + was_registred_by[current_injectable_id] + ", so module " + current_module_id + " can not be executed.");
					}
				}
				if(can_execute){
					module_ids_in_order.push(current_module_id);
					registred_ids.add(current_module_id);
					registered_injectables.addAll(current_module_info.defines);
					for(var injectable in current_module_info.defines){
						was_registred_by[current_module_info.defines[injectable]] = current_module_id;
//						console.log(current_module_info.defines[injectable] + " was registred by " + current_module_id)
					}
...
//		for (var i in registered_injectables.toArray()){
//			console.log(i + ' was registred by ' + was_registred_by[i]);
//		}
...
//it takes array of modules and search for unresolvable dependencies
	var message = "There are problems with modules:";
	var has_unresolved = false;
	for(var i in modules){
		for(var j in modules){
			console.log('Porównuję');
			console.log(intersection_of_arrays(modules[i].requires, modules[j].defines));
			console.log(intersection_of_arrays(modules[j].requires, modules[i].defines));
			console.log(i!=j && intersection_of_arrays(modules[i].requires, modules[j].defines) && intersection_of_arrays(modules[j].requires, modules[i].defines));
