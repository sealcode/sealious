...
io.on('connection', function(socket) {
		...
		*/
        ...
        socket.on("request", function(data){
        	...
        	var request_id = data.request_id;
        	console.log("received reqest to call ", method_name, ", with args", data.arguments);
        	//DispatcherDistributedBIZ[method_name].apply(DispatcherDistributedBIZ, data.arguments).then(function(response){
        	//DispatcherDistributedBIZ[method_name].apply(DispatcherDistributedBIZ, {0: "nono"}).then(function(response){
...
var DispatcherDistributedBIZ = new function(){
	...
	this.database_query = function(collection_name, mode, query, options, output_options){
		...
		console.log("@@@@@@@@got request to query database with mode ", mode);
		return new Promise(function(resolve, reject){
			...
			if(mode=="delete"){
				...
				request.del(url, {form:{json_encoded_payload:payload_string}}, function(err, res, body){
					if(err){
						throw new PrometheusError(err, "update_error", "database-accessor");
						reject(err);
					}else{
...
//in local mode		
	...
	/* ODKOMENTOWAĆ  
	*/
	this.resources_update = function(type_name, body){
	
		return ResourceManager.update_resource(type_name, body, this);
	}
